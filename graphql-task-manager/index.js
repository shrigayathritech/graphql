...
const root = {
  ...
  createTask: ({ title, description, status }) => {
    const task = { id: idCounter++, title, description, status };
    tasks.push(task);
    return task;
  },
...
app.use(
  ...
  graphqlHTTP({
    schema: schema,
    rootValue: root,
    graphiql: true, // Enable the GraphiQL interface for testingPS 
  })
);
...
//E:\foss4> cd graphql-task-manager
    //PS E:\foss4\graphql-task-manager> npm init -y
    //PS E:\foss4\graphql-task-manager> node index.js